...
def build_ceo_tasks(agents: dict) -> dict:
    ...
    tarefa_fazer = Task(
        description=(
            ...
            "── AUTOEVOLUÇÃO (última etapa, e a mais importante a longo prazo) ──\n"
            "Você é responsável por deixar o time melhor do que encontrou.\n\n"
            "4. Chame historico_evolucao pra ver o que já foi ajustado antes.\n"
            "5. Olhe este ciclo com honestidade: algum agente do time entregou "
            "mal? O Analista trouxe número irrelevante? O CEO escolheu uma "
            "prioridade fraca ou genérica? O Relator escreveu difícil demais? "
            "Você mesmo entregou conselho em vez de trabalho pronto?\n"
            "6. SE — e somente se — você identificar um padrão que já apareceu "
            "mais de uma vez (confira na memória e no histórico), chame "
            "evoluir_prompt UMA vez, corrigindo o goal ou o backstory do "
            "agente responsável. Escreva o campo COMPLETO, não um trecho, e "
            "explique no motivo qual padrão você observou.\n"
            "7. Se foi um ciclo normal, sem padrão claro de falha, NÃO mude "
            "nada e diga explicitamente 'sem evolução necessária neste ciclo'. "
            "Mexer no time por mexer é pior que não mexer: sem estabilidade "
            "não dá pra saber o que funcionou."
        ),
        expected_output=(
            "O trabalho pronto (texto/proposta completa) + confirmação de que "
            "os recados foram respondidos, o ciclo foi registrado, a memória "
            "atualizada, e a conclusão sobre autoevolução (o que foi ajustado "
            "no time e por quê, ou por que nada precisou mudar)."
        ),
        agent=agents["executor"],
    ...
    tarefa_contar = Task(
        description=(
            ...
            "6. <b>O time mudou em algo?</b> se algum agente foi ajustado "
            "neste ciclo, conte em uma frase o que mudou e por quê. Se não "
            "mudou nada, escreva 'o time seguiu igual hoje'\n"
            "7. <b>Amanhã:</b> a próxima prioridade\n\n"
